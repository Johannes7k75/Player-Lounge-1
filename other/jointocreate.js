...
//voice state update event to check joining/leaving channels
  ...
  if (!oldState.channelID && newState.channelID) {
    if(newState.channelID !== config.discord.JOINTOCREATECHANNEL) return;  //if its not the jointocreatechannel skip
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    if (oldState.channelID !== newState.channelID) {
      ...
      if(newState.channelID===config.discord.JOINTOCREATECHANNEL) 
      //make a new channel
      ...
      if (jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
        ...
        if (vc.members.size < 1) { 
          ...
          jointocreatemap.delete(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`); 
         //log it 
         client.channels.cache.get('862053601815560243').send(':x: '+oldState.member.user.username+'#'+oldState.member.user.discriminator+` - deleted ${vc.name}\n<t:${Math.floor(Date.now() / 1000)}>`)
        //delete the room
          return vc.delete(); 
      }
      else {
    ...
    async function jointocreatechannel(user) {
      ...
      await user.guild.channels.create(`${user.member.user.username}'s Lounge`, {
        type: 'voice',
        ...
        user.setChannel(vc);
        client.channels.cache.get('862053601815560243').send(':white_check_mark: '+user.member.user.username+'#'+user.member.user.discriminator+` - created ${vc.name}\n<t:${Math.floor(Date.now() / 1000)}>`)
