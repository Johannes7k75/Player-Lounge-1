        ...
        await mongo().then(async (mongoose) => {
            try {
                if (message.mentions.users.size > 0) {
                    ...
                    var users = await profileSchema.find({ guildId: message.mentions.members.first().guild.id }).sort([['totalxp', 'descending']]).exec();
                    ...
                    var users = await profileSchema.find({ guildId: message.member.guild.id }).sort([['totalxp', 'descending']]).exec();
